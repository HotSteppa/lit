...
#	* use command: "streamlit run lit.py"
...
#############################################################333333
##ALSO CHECK THIS SHIT OUT HERE DATA IMPORTANT

	# 5 --- You can work with data, change it and filter it as you always do using Pandas or any other library
	start_station_list = ['Stock'] + data['Call Ask Price'].unique().tolist()
	end_station_list = ['Stock'] + data['Call Bid Price'].unique().tolist()
	...
	if s_station != 'All':
		display_data = data[data['Call Bid Price'] == s_station]
...
#######################################3
	...
	multi_select = st.multiselect('Which start station would you like to see?',start_station_list, key='start_station', default=['Stock','Stock'])
	...
	slider_input = st.slider('How long should the trip be?', int(data['Call Ask Price'].min()/3600), int(data['Call Ask Price'].max()/3600), 25)
	...
	user_type = data['Stock'].value_counts().reset_index()
	...
	start_location = data['Stock'].value_counts()
...
stock_1 = yf.Ticker(text_input)

info = stock_1.info
chain = stock_1.cashflow
option = st.beta_container()



with option:
	st.header("hello")
	st.write("this is where the data is mentioned and then diplayed")
